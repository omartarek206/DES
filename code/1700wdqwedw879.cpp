...
int main() {
    ...
    string data = hex_to_bin(input[1]);
    string key = hex_to_bin(input[2]);
    string cipher = "";

    transform(input[0].begin(), input[0].end(), input[0].begin(), tolower);
    
    if(input[0] == "encrypt")          cipher = encrypt(key, data);
    else if (input[0] == "decrypt")    cipher = decrypt(key, data);
    else
        cout << "Recheck the desired operation or enter \"n\" to terminate the program";
